...
// TempDir is temp files directory
const TempDir = "./tmp/"
...
// Returns pointer to os.File and error if occured
func (s *DataStorage) CreateTempFile(name string) (*os.File, error) {
	file, err := os.OpenFile(TempDir+name,
...
func (s *DataStorage) OpenTempFile(name string) (*os.File, error) {
	file, err := os.OpenFile(TempDir+name,
...
func (s *DataStorage) QueryFileId(cond map[string]interface{}) (string, error) {
	c := s.db.C(FileMetaCollection)

	result := FileProperties{}
	if err := c.Find(cond).One(&result); err != nil {
		log.Println("In QueryMetaGeneric: ", err)
		return "", err
	}
	return result.Reference.Hex(), nil
}

// StoreFromDisk stores disk file in GridFS
// If local file's sha-256 is not equal to sha-256 value in FileMeta,
// error is returned
func (s *DataStorage) StoreFromDisk(name string, meta *FileMeta) (string, error) {
	path := "./tmp/" + name
	if strings.EqualFold(Sha256(path), meta.Hash) {
		...
		return gridFile.Id().(bson.ObjectId).Hex(), nil
	}
